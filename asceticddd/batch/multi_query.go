...
type MultiQueryBase struct {
	...
	placeholders string
	params       [][]any
	results      []*deferred.DeferredImp[session.Result]
	re           *regexp.Regexp
	replacement  string
	concat       string
}
...
func (q *MultiQueryBase) Exec(query string, args ...any) (deferred.Deferred[session.Result], error) {
	...
	r := &deferred.DeferredImp[session.Result]{}
...
func (q MultiQuery) Evaluate(s session.Session) (session.Result, error) {
	var errs error
	r, err := s.(session.DbSession).Connection().Exec(q.sql(), q.flatParams()...)
	if err != nil {
		return nil, err
	}
	for i := range q.results {
		d := q.results[i]
		d.Resolve(result.NewResult(0, 0))
...
func (q AutoincrementMultiInsertQuery) Evaluate(s session.Session) (session.Result, error) {
	var id int64
	var errs error
	rows, err := s.(session.DbSession).Connection().Query(q.sql(), q.flatParams()...)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		d := q.results[i]
		d.Resolve(result.NewResult(id, 0))
		occurredErr := d.OccurredErr()
		if occurredErr != nil {
			errs = multierror.Append(errs, occurredErr)
		}
