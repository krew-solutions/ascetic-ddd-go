...
4. Теперь из этой же директории выполнить ```docker compose -p 'ascetic-ddd-go' up -d```
...
6. Далее нужно открыть контейнер с `PostgreSQL` - `db-ascetic-ddd-go` в консоли (ну или зайти в контейнер через `exec`):
![Alt text](readme_images/docker_readme/db-ascetic-ddd-go-container-exmpl.png)
...
1. Открыть в браузера контейнер с `pgAdmin` - `pgadmin-ascetic-ddd-go`
![Alt text](readme_images/docker_readme/pgadmin-ascetic-ddd-go-menu-open-in-browser.png)
2. Откроется страница с предложением ввести пароль:
![Alt text](readme_images/docker_readme/pgadmin-ascetic-ddd-go-first-open-in-browser.png)
- пароль вводим из `.env`, указанный в параметре `DB_PASSWORD`
![Alt text](readme_images/docker_readme/pgadmin-ascetic-ddd-go-success-enter-pass-exmpl.png)
3. Теперь нужно настроить сервер, нажимаем на иконку `Add New Server`
- вводим имя сервера
![Alt text](readme_images/docker_readme/pgadmin-ascetic-ddd-go-name-server-exmpl.png)
- вводим настройки подключения
![Alt text](readme_images/docker_readme/pgadmin-ascetic-ddd-go-connection-settings-exmpl.png)
- если все успешно, сервер будет создан и будет доступна БД и ее таблицы
![Alt text](readme_images/docker_readme/pgadmin-ascetic-ddd-go-success-connect-exmpl.png)

# Проверка правильности развертывания
1. Если вышеперечисленные шаги выполнены верно, можно проверить пройдут ли тесты
2. Заходим в контейнер `app-ascetic-ddd-go` и выполняем команду `make test`:
![Alt text](readme_images/docker_readme/app-ascetic-ddd-go-success-test-exmpl.png)
3. И проверим таблицу `member`:
![Alt text](readme_images/docker_readme/app-ascetic-ddd-go-db-table-member-exmpl.png)
